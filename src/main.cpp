...
struct Net {
    ...
    */
    ...
    void train(vector<int> x, vector<int> y, int num_epochs, double learning_rate) {
        ...
        for(int n = 0; n < num_epochs; n++) {
            ...
            cout << "after epoch " << n << endl;

            for(int i = 0; i < SZ; i++) {
                cout << fixed << setprecision(9) << H[2][i] << " ";
            }    
            cout << endl;
...
int main() {
    Dataset ds = load("../data/mnist/train-images.f32", "../data/mnist/train-labels.u8", 1);
    ...
    for(auto x:ds.labels) {
        cout << int(x) << " ";
    }
    cout << endl;

    cout << ds.images.size() << " " << ds.labels.size() << endl;
